...
sid=1849
body=[{"content":"Проверка связи №2 ✅ Это я, Claude, шлю через API на твой личный чат (id 1849, Волшебные пилюли). Если видишь — отправка работает на тебя лично. Можно удалить."}]
for ct in ("application/vnd.api+json","application/json"):
    ...
    print(f"POST messages [{ct}] -> {r.status_code}: {r.text[:200]}")
    if r.ok: break
